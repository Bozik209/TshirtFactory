...
function getData() {
  fetch("https://api.scalablepress.com/v2/categories/sweatshirts")
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
      const products = data.products;
      createImage(products);
    });
}

function createImage(products) {
  products.forEach((e) => {
    // console.log(e);
    // console.log(e.image.url);
    if (e.image.url !== undefined) {
      addElement(e.image.url);
    }
  });
}
...
function addElement(src) {
  ...
  x.setAttribute("alt", "Tshirt");
  x.setAttribute("class", "img_shirt");
  shirt.appendChild(x);
}
...
// ********** MAP ************
var map = L.map("map").setView([23.786791, 121.010256], 8);
